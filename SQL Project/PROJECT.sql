...
--Joining the Region and Land Area tables but onlt displaying the country, land area and income group for countries that fit the range --
...
SELECT R.income_group,ROUND(AVG(forest_area_sqkm), 0) AS 'AVG FOREST AREA'
FROM Region R FULL JOIN Forest_Area F ON R.country_name = F.country_name
GROUP BY R.income_group
HAVING income_group IN ('upper middle income','high income','low income','lower middle income')
ORDER BY 'AVG FOREST AREA';
